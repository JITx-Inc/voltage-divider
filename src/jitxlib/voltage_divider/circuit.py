...
def _warn_name_ignored(name: Optional[str]) -> None:
    if name is not None:
        warnings.warn(
            "The `name` argument is deprecated and ignored: JITX 4.2+ forbids creating "
            "classes dynamically during instantiation, so the circuit can no longer be "
            "renamed per instance. The circuit is always named 'VoltageDividerCircuit'.",
            DeprecationWarning,
            stacklevel=3,
        )
...
def voltage_divider(
    ...
    .. deprecated::
        The `name` argument is ignored. JITX 4.2+ disallows creating classes
        dynamically during instantiation, so the per-instance rename is gone.
    """
